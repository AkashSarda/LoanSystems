...
function verify(address,hashSha3){
  instance.verify(hashSha3,{from:address},function(err,res){
		console.log(res);
		var number = web3.eth.getTransactionCount(address);
console.log(number);
		//console.log(web3.eth.getTransactionReceipt(address));
		$("#getVerifyResult").html("Verified Successfully..");
  })
...
window.onload = function() {
  ...
  initContract();
    ...
    var hashSha3 = web3.sha3(FirstName + LastName + HashValue + RegDate + SocialSecId);
      ...
      $("#getVerifyResult").show();
    	verify(EthAddress,hashSha3);
			console.log("success...!!!");
		}
		else{
